...
func (d *daemon) handleHistory(m ipc.HistoryRequestMsg) {
	if m.Op == "history.unified" {
		ctx, cancel := context.WithTimeout(context.Background(), 2500*time.Millisecond)
		defer cancel()
		result := d.unifiedRecent(ctx, m.Limit)
		items := make([]ipc.HistoryInfo, len(result.Items))
		for i, item := range result.Items {
			items[i] = ipc.HistoryInfo{
				Track: trackInfo(item.Track, i, 0), PlayedAt: item.PlayedAt.UTC().Format(time.RFC3339),
				Sources: item.Sources,
			}
		}
		reply(m.Reply, ipc.Response{
...
func (d *daemon) unifiedRecent(ctx context.Context, limit int) recent.Result {
	for _, entry := range d.providers {
		if source, ok := entry.Provider.(recent.UnifiedSource); ok {
			return source.UnifiedRecent(ctx, limit)
		}
	}
	items, err := (recent.HistorySource{Store: d.historyStore}).Recent(ctx, limit)
