...
Valid values: `spotify` (default), `radio`, `navidrome`, `plex`, `jellyfin`, `emby`, `qobuz`, `soundcloud`, `netease`, `yt`, `youtube`, `ytmusic`. If the configured provider is not registered (for example Spotify without credentials), cliamp falls back to the first available provider.
