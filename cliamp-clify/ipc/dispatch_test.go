...
func TestDispatchLyricsAndHistory(t *testing.T) {
	for _, request := range []Request{{Cmd: "lyrics"}, {Cmd: "history", Limit: 25}, {Cmd: "history.unified", Limit: 20}, {Cmd: "history.clear"}} {
		disp := &captureDispatcher{autoReply: Response{OK: true}}
		resp := newTestServer(disp).dispatch(request)
		if !resp.OK {
			t.Fatalf("%s: OK = false, err=%q", request.Cmd, resp.Error)
		}
		if request.Cmd == "history.clear" || request.Cmd == "history.unified" {
			got, ok := disp.last.(HistoryRequestMsg)
			if !ok || got.Op != request.Cmd || got.Limit != request.Limit {
				t.Fatalf("got %#v, want %s", disp.last, request.Cmd)
			}
		}
	}
}
