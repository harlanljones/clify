...
func TestLoadOmarchyPrefersStatePathOverLegacyConfig(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)

	write := func(dir, green string) {
		t.Helper()
		if err := os.MkdirAll(dir, 0o755); err != nil {
			t.Fatalf("MkdirAll: %v", err)
		}
...
`
	...
	write(filepath.Join(home, ".config", "omarchy", "current", "theme"), "#aaaaaa")
	write(filepath.Join(home, ".local", "state", "omarchy", "current", "theme"), "#6b7f63")
	...
	if th.Green != "#6b7f63" {
		t.Fatalf("Green = %q, want state-path #6b7f63", th.Green)
	}
}

func TestParseOmarchyAetherCommentedFile(t *testing.T) {
	input := `# Generated by Aether.
mode = "dark"
accent = "#777980"
foreground = "#989ba2"
bright_fg = "#9ea1a6"
...
`
	...
	if th.Accent != "#777980" || th.Green != "#6b7f63" || th.Yellow != "#87775b" || th.Red != "#986f6d" {
		t.Fatalf("parsed = %+v", th)
	}
	if th.BrightFG != "#9ea1a6" {
		t.Fatalf("BrightFG = %q, want #9ea1a6", th.BrightFG)
	}
}
