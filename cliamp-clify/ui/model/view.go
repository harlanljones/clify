...
func (m Model) renderTitle() string {
	title := titleStyle.Render("C L I F Y")
	label := m.focus.label()
	...
	if m.provSearch.active {
		...
		if isRadio {
			...
			if m.provSearch.query == "" {
				...
				hasSections := slices.ContainsFunc(m.providerLists, func(p playlist.PlaylistInfo) bool {
					return p.Section != ""
				})
				scroll := m.provSearch.scroll
				if scroll >= len(m.provSearch.results) {
					scroll = max(0, len(m.provSearch.results)-1)
				}
				if m.provSearch.cursor < scroll {
					scroll = m.provSearch.cursor
				}

				// Reserve one line each for the query and result-count rows;
				// section headers consume viewport space like the unfiltered
				// list, so advance until the cursor's rendered row fits.
				maxRows := visibleBudget - 2
				for scroll < len(m.provSearch.results)-1 &&
					m.provSearchRenderedRows(scroll, m.provSearch.cursor, hasSections) > maxRows {
					scroll++
				}

				prevSection := ""
				rowLimit := visibleBudget - 1 // reserve the result-count line
				for j := scroll; j < len(m.provSearch.results); j++ {
					idx := m.provSearch.results[j]
					p := m.providerLists[idx]

					if hasSections && p.Section != prevSection {
						if len(lines)+2 > rowLimit {
							break // never let a header evict its own group
						}
						lines = append(lines, dimStyle.Render(labeledSeparator("  ", p.Section)))
						prevSection = p.Section
					}
					if len(lines) >= rowLimit {
						break
					}
		...
		// Header-aware scrolling for every provider: section header rows
		// consume viewport space, so advance the window until the logical
		// cursor's rendered position fits the visible budget.
...
// provSearchRenderedRows counts the viewport rows (results plus the section
// headers between them) rendered from a filter-results scroll offset through
// the cursor. Mirrors providerRowsFromScroll for filtered views.
func (m Model) provSearchRenderedRows(scroll, cursor int, hasSections bool) int {
	results := m.provSearch.results
	if len(results) == 0 || cursor < scroll || scroll < 0 || cursor >= len(results) {
		return 0
	}
	...
	prevSection := ""
	if scroll > 0 {
		prevSection = m.providerLists[results[scroll-1]].Section
	}
	for i := scroll; i <= cursor; i++ {
		section := m.providerLists[results[i]].Section
		if hasSections && section != "" && section != prevSection {
