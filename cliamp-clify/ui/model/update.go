...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case playlistsLoadedMsg:
		...
		// The fresh list may be shorter than the previous one; keep cursor
		// and scroll inside bounds so sectioned rendering stays correct
		// (covers Ctrl+R refreshes).
		if m.provCursor >= len(m.providerLists) && len(m.providerLists) > 0 {
